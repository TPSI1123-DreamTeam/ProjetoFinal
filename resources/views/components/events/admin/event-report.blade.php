...
<div class="graphs">
    ...
    <div class="max-w-sm w-full bg-white rounded-lg shadow dark:bg-gray-800 p-4 md:p-6">
        <div class="flex justify-between border-gray-200 border-b dark:border-gray-700 pb-3">
            <dl>   
                @if($events->sum('amount') > $events->sum('services_amount'))                  

                <dt class="text-base font-normal text-gray-500 dark:text-gray-400 pb-1">Lucros</dt>
                <dd class="leading-none text-3xl font-bold text-gray-900 dark:text-white">{{ number_format(($events->sum('amount') - $events->sum('services_amount')), 2, ',', '.') }}€</dd>

                @else

                <dt class="text-base font-normal text-gray-500 dark:text-gray-400 pb-1">Prejuízos</dt>
                <dd class="leading-none text-3xl font-bold text-gray-900 dark:text-white">{{ number_format(($events->sum('amount') - $events->sum('services_amount')), 2, ',', '.') }}</dd>
                @endif
            </dl>
            <div>
